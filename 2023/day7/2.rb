...
def jokerize(hand)
    # check if hand as any Js
    if !hand.include?('J') || hand.count('J') == 5
        return hand
    end
    ...
    max_occurring_card = hand_without_jokers.chars.max_by { |char| hand_without_jokers.count(char) }

    return hand.gsub('J', max_occurring_card)
end
...
sorted_hands = hands.sort do |hand_a, hand_b|
    power_of_a = get_hand_power(jokerize(hand_a), hand_powers)
    power_of_b = get_hand_power(jokerize(hand_b), hand_powers)
