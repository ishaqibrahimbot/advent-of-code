...
fn bring_sand_to_rest(map: &mut HashMap<(i32, i32), Point>, lowest_rock: i32) -> bool {
    ...
    let is_source_blocked = map.get(&(starting_x, starting_y));

    if let Some(occupant) = is_source_blocked {
        return false;
    }
    ...
    loop {
        ...
        if first_position.1 == lowest_rock + 2 {
            // println!("Hit the floor. Sand settled ");
