...
impl Map {
    ...
    pub fn cleanup_state(&mut self) {
        self.state = self.state.clone().into_iter().filter(|point| point.1 > (self.highest_y - 10)).collect();
    }
}
...
fn main() {
    ...
    let mut directions_copy = directions.clone();
    loop {
        // println!("iteration: {}", i);
        i += 1;
        ...
        if num_rocks % 10000 == 0 {
            println!("{num_rocks}");

        }

        if directions_copy.len() == 0 {
            // println!("{:?}", game.rock_in_motion.as_ref().unwrap().rock_type);
            // println!("It's empty: {i}");
            if game.rock_in_motion.as_ref().unwrap().rock_type == RockType::Minus {
                println!("2 conditions met");
            }
            directions_copy = directions.clone();
        }

        let direction = directions_copy.pop_front().unwrap();
